...
func ntoh8(rd io.Reader) (i uint64, err error) {
	var (
		n   int
...
func ntoh4(rd io.Reader) (i uint32, err error) {
	var (
		n   int
...
// 从rd中读取2个字节，转换为int16
func ntoh2(rd io.Reader) (i uint16, err error) {
	var (
		n   int
	...
	i = uint16(uint16(buf[0])<<8 + uint16(buf[1]))
